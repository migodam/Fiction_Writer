...
5b. (Optional) If `state["planner_proposal"]` or `context["planner_proposal"]` is present: `validate_planner_proposal()` runs first — invalid proposal → `hard_fail` before any extraction begins. If valid, `planner_proposal_to_import_plan()` converts it to an `ImportPlan`, replacing the step-5 deterministic plan. The validation result is stored as `state["planner_proposal_validation"]`. When no proposal is present, step 5 runs exactly as before with no behavioral change.
