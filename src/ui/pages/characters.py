...
def render(memory: ProjectMemory):
    ...
    if candidates:
        with st.expander(f"Review AI-Generated Candidates ({len(candidates)})", expanded=True):
            for can in candidates:
                is_updated = can.get("ui_metadata", {}).get("is_new_update", False)
                c1, c2, c3 = st.columns([4, 1, 1])
                label = f"✨ **{can['name']}**" if is_updated else f"**{can['name']}**"
                c1.write(f"{label} - {can.get('role', 'Potential Character')}")
                if c2.button("Confirm", key=f"conf_{can['id']}"):
                    memory.confirm_character(can['id'])
                    memory.clear_update_flag("character", can['id'])
                    st.rerun()
    ...
    with col_nav:
        ...
        for char in active:
            is_updated = char.get("ui_metadata", {}).get("is_new_update", False)
            btn_label = f"✨ {char['name']}" if is_updated else char['name']
            
            if is_updated:
                st.markdown('<div class="updated-highlight">', unsafe_allow_html=True)
            
            if st.button(btn_label, key=f"nav_c_{char['id']}", use_container_width=True):
                st.session_state.nl_active_char_id = char["id"]
                if is_updated:
                    memory.clear_update_flag("character", char["id"])
                    st.rerun()
    ...
    with col_main:
        ...
        if cid == "NEW":
            ...
            if char:
                ...
                # IDE Style Tags & Aliases
                tags = char.get("tags", [])
                aliases = char.get("aliases", [])
                meta_html = ""
                if aliases: meta_html += f"<span style='color: #888; font-size: 14px; margin-right: 10px;'>AKA: {', '.join(aliases)}</span>"
                if tags:
                    for t in tags: meta_html += f"<span class='ide-tag'>#{t}</span>"
                if meta_html:
                ...
                with tabs[0]:
                    ...
                    bg_val = char.get("background") or char.get("description") or ""
                    char["background"] = st.text_area("Background / History", value=bg_val, height=200)
                    char["description"] = char["background"] # Sync
                ...
                with tabs[2]:
                    ...
                    for e in sorted(involved_events, key=lambda x: str(x.get("time"))):
                        ...
                        st.caption(f"Location: {e.get('location')} | Consequences: {e.get('consequences', 'None recorded')}")
                        st.write("---")
                ...
                with tabs[3]:
                    ...
                    if st.button("Generate POV Insights"):
                        ...
                        if involved_events:
                            ...
                            insights = workflow.generate_pov_timeline(char["name"], involved_events[:5]) 
                            for ins in insights:
