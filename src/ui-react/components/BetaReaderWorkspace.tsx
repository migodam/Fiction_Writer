        ...
        <div className="border-b border-border bg-bg-elev-2 p-4">
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              <div className="text-sm font-black text-text">{zh ? '读者画像' : 'Reader Personas'}</div>
            </div>
            <Users size={16} className="text-brand" />
          </div>
          <div className="grid gap-2">
            <input value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} placeholder={zh ? '画像名称' : 'Persona name'} className="rounded-2xl border border-border bg-bg px-4 py-3 outline-none" />
            <input value={draft.profile} onChange={(event) => setDraft((current) => ({ ...current, profile: event.target.value }))} placeholder={zh ? '关注点' : 'What do they care about?'} className="rounded-2xl border border-border bg-bg px-4 py-3 outline-none" />
            <button
              type="button"
              ...
              onClick={() => {
                ...
                addBetaPersona({
                  ...
                  archetype: zh ? '自定义' : 'Custom',
                  profile: draft.profile.trim() || (zh ? '通用读者画像。' : 'General reader persona.'),
                  tone: 'balanced',
                ...
                setLastActionStatus(zh ? '读者画像已创建' : 'Persona created');
              }}
            >
              <Plus size={14} className="mr-2 inline" />
              {zh ? '新建画像' : 'Create Persona'}
            </button>
          </div>
        </div>
            ...
            <button key={persona.id} type="button" className={cn('mb-2 w-full rounded-2xl border px-4 py-4 text-left transition-colors', activePersonaId === persona.id ? 'border-brand bg-active text-text' : 'border-transparent text-text-2 hover:bg-hover')} onClick={() => setActivePersonaId(persona.id)} data-testid={`beta-persona-${persona.id}`}>
              <div className="mb-2 flex items-center justify-between gap-3">
                ...
                <button type="button" className="rounded border border-red/40 p-1 text-red" onClick={(event) => {
                  ...
                  setLastActionStatus(zh ? '读者画像已删除' : 'Persona deleted');
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar p-10">
        <div className="mx-auto max-w-6xl">
          <div className="mb-8 grid gap-4 md:grid-cols-4">
            <MetricCard label={zh ? '互动感' : 'Engagement'} value={activeRun?.aggregate.engagement || 0} icon={<TrendingUp size={14} />} />
            <MetricCard label={zh ? '留存' : 'Retention'} value={activeRun?.aggregate.retention || 0} icon={<UserCheck size={14} />} />
            <MetricCard label={zh ? '共鸣' : 'Resonance'} value={activeRun?.aggregate.resonance || 0} icon={<Heart size={14} />} />
            <MetricCard label={zh ? '画像数' : 'Personas'} value={betaPersonas.length} icon={<Users size={14} />} />
          </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[1fr_0.85fr]">
              <div>
                <div className="rounded-3xl border border-border bg-bg-elev-1 p-5">
                  <div className="mb-3 text-[10px] font-black uppercase tracking-[0.18em] text-text-3">{zh ? '画像说明' : 'Profile'}</div>
                  <div className="text-sm leading-relaxed text-text-2">{activePersona?.profile || (zh ? '选择一个画像查看反馈。' : 'Select a persona to review feedback.')}</div>
                </div>
              ...
              <div>
                <div className="rounded-3xl border border-border bg-bg-elev-1 p-5">
                  <div className="mb-4 flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.18em] text-text-3">
                    ...
                    {zh ? '整体评分' : 'Aggregate Scores'}
                  </div>
                  <div className="space-y-4">
                    <AggregateRow label={zh ? '互动感' : 'Engagement'} value={Math.round(aggregate.engagement / Math.max(betaRuns.length, 1))} />
                    <AggregateRow label={zh ? '留存' : 'Retention'} value={Math.round(aggregate.retention / Math.max(betaRuns.length, 1))} />
                    <AggregateRow label={zh ? '共鸣' : 'Resonance'} value={Math.round(aggregate.resonance / Math.max(betaRuns.length, 1))} />
                    <AggregateRow label={zh ? '节奏' : 'Pacing'} value={Math.round(aggregate.pacing / Math.max(betaRuns.length, 1))} />
                    <AggregateRow label={zh ? '一致性' : 'Consistency'} value={Math.round(aggregate.consistency / Math.max(betaRuns.length, 1))} />
                  </div>
                </div>
              </div>
            </div>
