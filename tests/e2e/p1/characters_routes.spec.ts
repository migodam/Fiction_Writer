...
test.describe('characters route contract', () => {
  test('candidate queue is reachable by route and section selector', async ({ page }) => {
    ...
    await expect(page.getByTestId('candidate-card-cand_mina')).toBeVisible();
  });

  test('character profile route loads the selected record', async ({ page }) => {
    await page.goto('/characters/profile/char_aria');

    await expect(page.getByTestId('character-name-input')).toHaveValue('Aria Solis');
    await expect(page.getByTestId('character-background-input')).toContainText('Ember Quarter');
    await expect(page.getByTestId('status-bar')).toContainText('Aria Solis');
  });
  ...
  test('invalid character profile shows not-found state and recovery action', async ({ page }) => {
    ...
    await expect(page.getByTestId('entity-not-found')).toContainText(/Entity not found|未找到该实体/);
