...
test.describe('W1 Import — Token / Cost card', () => {
  test('renders token/cost card with actual usage during running', async ({ page }) => {
    ...
    await expect(page.getByTestId('w1-import-budget')).toContainText('57.0k');
    await expect(page.getByTestId('w1-import-budget')).toContainText('9 calls');
    await expect(page.getByTestId('w1-import-budget')).toContainText('$3.00');
  });
