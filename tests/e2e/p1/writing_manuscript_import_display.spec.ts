...
test.describe('Writing workspace — manuscript node content readability', () => {
  test('clicking a scene node shows non-empty content via loadManuscriptNodeContent', async ({ page }) => {
    const fixture = makeManuscriptNodesFixture();
    const SAMPLE_TEXT = '韩立踏上修仙之路，历经千辛万苦。';
    ...
    for (const node of fixture.manuscriptNodes) {
      if ((node as any).type === 'scene_outline') {
        ...
        fakeFiles[filePath] = SAMPLE_TEXT;
      }
    }
    ...
    await page.addInitScript(
      ({ project, files, sampleText, bridgeMethods }) => {
        ...
        const fakePathModule = {
          // Preserve protocol double-slash (memory://, file://) while collapsing
          // any spurious double-slashes produced by joining path segments.
    ...
    // Verify actual sample text appears in the editor, not only non-zero word count
    await expect(page.getByTestId('manuscript-editor')).toContainText('韩立');
