...
## 2026-07-14 proposal graph closure

A second real acceptance attempt exposed an event-identity split: Timeline Architect retained canonical `event_c17b6a40`, but proposal writing ran another fuzzy-title dedupe and omitted that event after character backlinks had already been built. Three character proposals therefore referenced an event absent from the package.

The repair makes Timeline Architect the sole event identity authority and adds a deterministic typed proposal graph compiler. It applies explicit event remaps, distinguishes required structural edges from optional backlinks, validates canonical-or-created-in-package closure, handles legal backlink cycles with Tarjan SCCs, emits stable Kahn ordering, and removes any non-atomic staged package before Workbench review. The React validator now rejects updates as same-package producers and covers branch parents, fork/merge anchors, merge targets, and shared branches.

The preserved 10-chapter inbox was repaired without another API call: exactly three optional stale backlinks were removed, all 89 stale block markers were cleared, and `proposal_graph.json` records the normalization. Final evidence: `677` W1 Python tests pass; UI build/lint pass; all `24` import-package Playwright tests pass; Electron runtime smoke passes; real-artifact diagnostics exit `0` with dangling references `0`, stale block markers `0`, and every symptom flag false. The broader E2E run passed `238/250`; its 12 failures are confined to legacy `tests/e2e/smoke.spec.ts` assertions for selectors/routes that no longer exist in the active UI and are recorded as a separate test-maintenance baseline issue.
