...
| `sidecar/supervisor/quality.py` | New file — `evaluate_import_quality(state)` zero-cost quality rubric; includes world/person exact-name collision warnings and zero-cost prompt-window ledger estimates |
...
| `tests/test_w1_quality_rubric.py` | New file — 15 tests for `evaluate_import_quality()` |
...
Phase B (targeted):
```
tests/test_w1_quality_rubric.py    15 passed
